...
public class PokerDeck : Deck {
	...
	private void CheckHandPid( int playerId ) {
		var hand = this.playerHands[ playerId ];
	...
	// num suit
	private int NumSuit( IEnumerable<int> hand, char suit ) {
		var cnt=0;
		foreach( var cc in hand )
			if( this.cards[cc].suit == suit )
				cnt+=1;	
		return cnt;
	}
	...
	private bool IsSuit( IEnumerable<int> hand, char suit ) {
		foreach( var cc in hand )
			if( this.cards[cc].suit == suit )
				return true;
		return false;
	...
	public void DebPokerResults() {
		...
		foreach( var pl in this.playerHands	 ){
			...
			Console.WriteLine( this.results[cnt].name );
			Console.WriteLine( this.results[cnt].hc1 );
			Console.WriteLine( this.results[cnt].hc2 );
			cnt += 1;	
		}
	}
	...
	public void DebHands() {
		// player 1 
		List<int> hh = this.playerHands[0];
		int[] hands = { 0,11,10,12,9 };	
		for( var ii=0; ii<5; ii++ )
			hh[ii]=hands[ii];
		
		// player 2 
		hh = this.playerHands[1];
		int[] hands2 = { 0,13,26,39,9 };	
		for( var ii=0; ii<5; ii++ )
			hh[ii]=hands2[ii];
			
		// player 3 
		hh = this.playerHands[2];
		int[] hands3 = { 0,13,1,14,27 };	
		for( var ii=0; ii<5; ii++ )
			hh[ii]=hands3[ii];

		// player 4 
		hh = this.playerHands[3];
